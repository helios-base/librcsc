...
PlayerTypeAnalyzer::updateLastData()
{
    ...
    for ( CoachPlayerObject::Cont::const_iterator p = M_world.teammates().begin(),
              end = M_world.teammates().end();
          p != end;
          ++p )
    {
    ...
    for ( CoachPlayerObject::Cont::const_iterator p = M_world.opponents().begin(),
              end = M_world.opponents().end();
          p != end;
          ++p )
    {
        if ( (*p)->unum() < 1 || 11 < (*p)->unum() ) continue;
...
PlayerTypeAnalyzer::checkCollisions()
{
    ...
    for ( CoachPlayerObject::Cont::const_iterator o = opponents.begin(), o_end = opponents.end();
          o != o_end;
          ++o )
    {
        ...
        for ( CoachPlayerObject::Cont::const_iterator t = teammates.begin(), t_end = teammates.end();
              t != t_end;
              ++t )
        {
